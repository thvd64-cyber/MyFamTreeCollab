...
// Live search + Broers/Zussen lijst geïntegreerd met dataset, zonder tabel dependency
(function(){
...
let selectedHoofdId = null; // geselecteerde persoon (init bij load)
...
function buildTree(rootID){ 
    ...
    if(root.VaderID){ 
        ...
        if(v) parents.appendChild(createNode(v)); 
    }
    if(root.MoederID){ 
        const m = findPerson(root.MoederID); 
        if(m) parents.appendChild(createNode(m)); 
    }
    ...
    if(root.PartnerID){ 
        ...
        if(partner) rootWrapper.appendChild(createNode(partner)); 
    }
    ...
    renderSiblings(rootID); // update broer/zus lijst
}
...
function liveSearch(){
    ...
    results.forEach(p => { 
        ...
        row.addEventListener('click', () => {
            ...
            popup.remove(); 
            renderTree(); // update boom + B/Z lijst
        }); 
...
function refreshView(){ 
    ...
    if(!selectedHoofdId && dataset.length > 0){ 
        selectedHoofdId = dataset[0].ID; // default persoon bij load
    }
    renderTree(); 
}
